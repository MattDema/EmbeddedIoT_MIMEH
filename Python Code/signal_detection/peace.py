...
def peace(hand, cv2, frame, recorder):
    ...
    Args:
        ...
        recorder (SignalRecorder): The recorder used to save the video if a signal is detected.
    """
    ...
    if (hand.is_two and not hand.is_hand_closed
        ...
        # If not recording or the current signal type isn't "peace", start a new recording
        if not recorder.is_recording or recorder.current_signal_type != "peace":
        ...
        # If recording and the current signal type is "peace", stop the recording
        if recorder.is_recording and recorder.current_signal_type == "peace":
            recorder.stop_recording()
